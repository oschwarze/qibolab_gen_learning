...
from qibolab.pulses import PulseSequence, ReadoutPulse,Pulse
>>>>>>> da0bddaf5ad2c76501070fbe887faa25d2c940eb
...
class PulseSimulator(Controller):
	...
	PortType = None
	all_simulation_engines = {"Qutip": QutipSimulator,} # this might as well be acessible without having to construct an instance of PulseSimulator first
	def __init__(
		self,
		simulation_config: dict,
		model_config: dict,
	...
	def run_pulse_simulation(
		...
		# extract waveforms from pulse sequence
		pulse_compilation_method = self.simulation_config.get('pulse compilation method','PC interpolated')
		log.info(f'compiling pulses for simulation using method: {pulse_compilation_method}')
		if pulse_compilation_method == 'PC interpolated':
		...
		elif pulse_compilation_method == 'analytic':
			...
			raise ValueError(f'unkown pulse compulation method: {pulse_compilation_method}')
		# execute pulse simulation in emulator
		simulation_results = self.simulation_engine.qevolve(
			channel_waveforms, self.simulate_dissipation
		)
	...
	def play(
		...
		results["simulation"] = {
			...
			"simulation_time": times_dict["simulation_time"],
			"output_states": output_states,
			"output_times": times_dict['output_state_times']
		}
...
def stringify_pulse(pulse:Pulse,label:str=None) -> tuple[str,dict[str,float]]: 
		"""
		...
		if isinstance(pulse.shape,pulses.Drag):
			pass
		...
		elif isinstance(pulse.shape,pulses.Rectangular):
